...
function pitch(request) {
  ...
  const childCompiler = this._compilation.createChildCompiler(`${pluginName} ${request}`, outputOptions); // TODO simplify after drop  webpack v4
  ...
  childCompiler.runAsChild((error, entries, compilation) => {
    ...
    const addDependencies = dependencies => {
      ...
      for (const dependency of dependencies) {
        ...
        const {
          CssDependency
        } = (0, _utils.shared)(webpack, () => {
          return {};
        });

        if (!CssDependency) {
          throw new Error("You forgot to add 'mini-css-extract-plugin' plugin (i.e. `{ plugins: [new MiniCssExtractPlugin()] }`), please read https://github.com/webpack-contrib/mini-css-extract-plugin#getting-started");
        }
